import { toCamelCaseDeep } from "./cases";
import type {
  ExtraDataExtensions,
  MetaMessage,
  UnifiedMessage,
...
export function normalizeWebhook(payload: unknown): NormalizedWebhookResult {
  ...
  for (const entry of entries) {
    ...
    for (const change of changes) {
      ...
      const messageEchoes = Array.isArray(value.messageEchoes)
        ? value.messageEchoes
        : [];
      ...
      for (const call of calls) {
        ...
        if (
          typeof normalizedCall.wacid === "string" &&
          !normalizedCall.callId
        ) {
...
function normalizeMessage(message: MetaMessage): UnifiedMessage {
  ...
  if (
    normalized.order &&
    typeof (normalized.order as Record<string, unknown>).text !== "undefined"
  ) {
    const orderRecord = normalized.order as Record<string, unknown> & {
      text?: unknown;
    };
    ...
    if (orderText !== undefined) {
      ...
      ensureExtraData(normalized).orderText = orderText as string;
    }
  }
  ...
  if (
    ...
    typeof (normalized.interactive.nfmReply as Record<string, unknown>)
      .responseJson === "string"
  ) {
    const nfm = normalized.interactive.nfmReply as Record<string, unknown> & {
      responseJson?: string;
      name?: string;
    };
    ...
    if (typeof responseJson === "string" && responseJson.trim()) {
      try {
        ...
        const extra = ensureExtraData(normalized);
        extra.flowResponse = camel;
        if (typeof camel.flowToken === "string") {
          extra.flowToken = camel.flowToken as string;
        }
        if (typeof nfm.name === "string") {
          extra.flowName = nfm.name;
        }
      } catch {
  ...
  if (normalized.extraData && Object.keys(normalized.extraData).length === 0) {
    delete normalized.extraData;
  }
...
function applyDirection(
  ...
  const displayPhoneNumber =
    typeof metadata.displayPhoneNumber === "string"
      ...
      : undefined;
  const contextFrom =
    typeof message.context?.from === "string"
  ...
  const fromNorm =
    typeof message.from === "string"
      ? normalizeNumber(message.from)
      : undefined;
  const toNorm =
    typeof message.to === "string" ? normalizeNumber(message.to) : undefined;
  ...
  if (direction || isEcho) {
    const extraData = ensureExtraData(message); // renamed from kapso
    if (direction) {
      extraData.direction = direction;
    }
    if (isEcho) {
      extraData.source = "smb_message_echo";
    }
  }
}

// ✅ Renamed helper
function ensureExtraData(message: UnifiedMessage): ExtraDataExtensions {
  if (!message.extraData) {
    message.extraData = {} as ExtraDataExtensions;
  }
